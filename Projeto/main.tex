...
A transparência na administração pública brasileira passou por avanços institucionais significativos nas últimas décadas, impulsionada por marcos regulatórios fundamentais como a Lei de Responsabilidade Fiscal (Lei Complementar nº 101/2000) e a Lei de Acesso à Informação (Lei nº 12.527/2011 -- LAI). A promulgação dessas legislações consolidou o paradigma da transparência ativa, obrigando os órgãos governamentais de todas as esferas federativas a disponibilizarem, em meio eletrônico de amplo acesso público, dados em tempo real sobre a execução orçamentária e financeira do Estado.

Entretanto, a simples disponibilização bruta de microdados orçamentários nos portais de transparência não assegura, por si só, a efetividade do controle social e da fiscalização contábil. Na prática, a sociedade civil, jornalistas de dados e até mesmo auditores de órgãos de controle enfrentam o desafio da opacidade administrativa derivada do volume e da desestruturação das informações disponibilizadas.
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        \node (fase2) [box, below=of fase1] {\textbf{2. Coleta e Ingestão dos Microdados} \\ {\footnotesize Extração populacional integral do Portal do Acre (2020--2025, +429 mil registros)}};
        ...
        \node (fase5) [box, below=of fase4] {\textbf{5. Triagem Estatística Descritiva} \\ {\footnotesize Lei de Newcomb-Benford, Teste Qui-Quadrado ($\chi^2$) e desvios por $Z$-Score}};
...
A execução da esteira inicia-se pela compreensão das normas de execução orçamentária do setor público (Fase 1), seguida da coleta da totalidade dos microdados de despesa do Estado do Acre no período de 2020 a 2025 (Fase 2). Após a verificação exploratória de consistência (Fase 3) e o pré-processamento para limpeza de ruídos e extração de atributos numéricos (Fase 4), os dados passam pela filtragem estatística paramétrica da Lei de Benford e do $Z$-Score (Fase 5).
...
\begin{figure}[htbp]
    ...
    \fonte{Portal da Transparência do Governo (2026).}
\end{figure}

Como observado na Figura \ref{fig:portal_transparencia_excesso}, a consulta disponibiliza milhares de linhas contendo códigos orçamentários, favorecidos e históricos contábeis preenchidos em texto livre. Para um cidadão ou analista humano, a tentativa de identificar manualmente fracionamentos ilícitos de licitação (\textit{smurfing}), superfaturamentos ou concentrações de mercado em uma base com mais de 429 mil registros (como no caso do Estado do Acre entre 2020 e 2025) torna-se uma tarefa operacionalmente inviável.
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \textbf{2.5. Mineração de Texto em Históricos Contábeis:} Vetorização TF-IDF e agrupamento denso via DBSCAN para filtragem contextual de falsos positivos.
    ...
    \begin{itemize}
        ...
        \item \textbf{3.6. Escore Integrado de Risco e Validação dos Resultados:} Formulação da matriz de concordância multialgorítmica e auditoria de falsos positivos.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{4.2. Triagem Estatística Descritiva:} Resultados da aderência à Lei de Benford, testes $\chi^2$ e identificação de picos de $Z$-Score.
        \item \textbf{4.3. Detecção por Aprendizado Não Supervisionado:} Mapeamento de anomalias globais (ISO) e locais de densidade (LOF).
        \item \textbf{4.4. Agrupamento Semântico e Filtro de Falsos Positivos:} Resultados do TF-IDF/DBSCAN no isolamento de despesas normativas tabeladas (diárias e auxílios).
        \item \textbf{4.5. Estrutura de Redes e Oligopólios:} Projeção de grafos de fornecedores e diagnóstico do uso de Atas de Registro de Preço.
        \item \textbf{4.6. Desempenho do Escore Integrado de Risco:} Validação do ranking final de \textit{red flags} para suporte à decisão auditiva.
    \end{itemize}
